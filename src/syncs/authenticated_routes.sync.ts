...
export const GetUserLogsRequest: Sync = ({ request, userId, logs }) => ({
  when: actions([Requesting.request, {
    path: "/ExperienceLog/_get_user_logs",
    userId,
  }, { request }]),
  where: async (frames) => {
...
export const GetPlaceLogsRequest: Sync = (
  { request, userId, placeId, logs },
) => ({
  when: actions([Requesting.request, {
    path: "/ExperienceLog/_get_place_logs",
    userId,
    placeId,
  }, { request }]),
  where: async (frames) => {
    ...
    const result = await ExperienceLog._get_place_logs({
      userId: userIdValue,
...
export const GetAverageRatingRequest: Sync = (
  { request, userId, placeId, averageRating },
) => ({
  when: actions([Requesting.request, {
    path: "/ExperienceLog/_get_average_rating",
    userId,
    placeId,
  }, { request }]),
  where: async (frames) => {
    ...
    const userIdValue = originalFrame[userId];
    const placeIdValue = originalFrame[placeId];
    const result = await ExperienceLog._get_average_rating({
      userId: userIdValue,
      placeId: placeIdValue,
    });
    return new Frames({
...
export const GetTriedPlacesRequest: Sync = ({ request, userId, places }) => ({
  when: actions([Requesting.request, {
    path: "/ExperienceLog/_get_tried_places",
    userId,
  }, { request }]),
  where: async (frames) => {
    const originalFrame = frames[0];
    const userIdValue = originalFrame[userId];
    const result = await ExperienceLog._get_tried_places({
      userId: userIdValue,
    });
...
export const GenerateProfileSummaryRequest: Sync = ({ request, userId }) => ({
  when: actions([Requesting.request, {
    path: "/ExperienceLog/generate_profile_summary",
    userId,
  }, { request }]),
...
export const GenerateProfileSummaryResponse: Sync = ({ request, summary }) => ({
  when: actions(
    [Requesting.request, { path: "/ExperienceLog/generate_profile_summary" }, {
      request,
    }],
...
export const CreatePlaceRequest: Sync = (
  { request, name, address, coords, hours, priceRange, style, place },
) => ({
  when: actions([Requesting.request, {
    path: "/PlaceDirectory/create_place",
    name,
    address,
    coords,
    hours,
...
export const EditPlaceRequest: Sync = (
  { request, placeId, name, address, coords, hours, priceRange, style },
) => ({
  when: actions([Requesting.request, {
    path: "/PlaceDirectory/edit_place",
    placeId,
    name,
    address,
    coords,
...
export const DeletePlaceRequest: Sync = ({ request, placeId }) => ({
  when: actions([Requesting.request, {
    path: "/PlaceDirectory/delete_place",
    placeId,
  }, { request }]),
  then: actions(
...
export const UnsavePlaceRequest: Sync = ({ request, userId, placeId }) => ({
  when: actions([Requesting.request, {
    path: "/UserDirectory/unsave_place",
    userId,
    placeId,
  }, { request }]),
  then: actions(
...
export const UpdatePreferencesRequest: Sync = (
  { request, userId, newPrefs },
) => ({
  when: actions([Requesting.request, {
    path: "/UserDirectory/update_preferences",
    userId,
    newPrefs,
  }, { request }]),
  then: actions(
...
export const GetSavedPlacesRequest: Sync = ({ request, userId, placeIds }) => ({
  when: actions([Requesting.request, {
    path: "/UserDirectory/_get_saved_places",
    userId,
  }, { request }]),
  where: async (frames) => {
    const originalFrame = frames[0];
...
export const GetRecommendationsRequest: Sync = (
  { request, userId, places },
) => ({
  when: actions([Requesting.request, {
    path: "/RecommendationEngine/get_recommendations",
    userId,
  }, { request }]),
  where: async (frames) => {
    const originalFrame = frames[0];
    const userIdValue = originalFrame[userId];
    const result = await RecommendationEngine.get_recommendations({
      userId: userIdValue,
    });
    return new Frames({ ...originalFrame, [places]: result.places });
  },
...
export const RefreshRecommendationsRequest: Sync = (
  {
    request,
    userId,
    savedPlaces,
  ...
  when: actions([Requesting.request, {
    path: "/RecommendationEngine/refresh_recommendations",
    userId,
    savedPlaces,
    preferences,
    triedPlaces,
  ...
  then: actions(
    [RecommendationEngine.refresh_recommendations, {
      userId,
      savedPlaces,
      preferences,
      triedPlaces,
...
export const ComputeSuggestionsRequest: Sync = (
  {
    request,
    userId,
    savedPlaces,
  ...
  when: actions([Requesting.request, {
    path: "/RecommendationEngine/compute_suggestions",
    userId,
    savedPlaces,
    preferences,
    triedPlaces,
  ...
  then: actions(
    [RecommendationEngine.compute_suggestions, {
      userId,
      savedPlaces,
      preferences,
      triedPlaces,
...
export const ClearRecommendationsRequest: Sync = ({ request, userId }) => ({
  when: actions([Requesting.request, {
    path: "/RecommendationEngine/clear_recommendations",
    userId,
  }, { request }]),
  then: actions(
...
export const GetUserRecommendationsRequest: Sync = (
  { request, userId, places },
) => ({
  when: actions([Requesting.request, {
    path: "/RecommendationEngine/_get_user_recommendations",
    userId,
  }, { request }]),
  where: async (frames) => {
    const originalFrame = frames[0];
    const userIdValue = originalFrame[userId];
    const result = await RecommendationEngine._get_user_recommendations({
      userId: userIdValue,
    });
    return new Frames({ ...originalFrame, [places]: result.places });
  },
...
export const GetLastUpdatedRequest: Sync = (
  { request, userId, lastUpdated },
) => ({
  when: actions([Requesting.request, {
    path: "/RecommendationEngine/_get_last_updated",
    userId,
  }, { request }]),
  where: async (frames) => {
    const originalFrame = frames[0];
    const userIdValue = originalFrame[userId];
    const result = await RecommendationEngine._get_last_updated({
      userId: userIdValue,
    });
...
export const GetRecommendationsWithinRequest: Sync = (
  { request, userId, allowedPlaces, places },
) => ({
  when: actions([Requesting.request, {
    path: "/RecommendationEngine/get_recommendations_within",
    userId,
    allowedPlaces,
  }, { request }]),
  where: async (frames) => {
    ...
    const result = await RecommendationEngine.get_recommendations_within({
      userId: userIdValue,
