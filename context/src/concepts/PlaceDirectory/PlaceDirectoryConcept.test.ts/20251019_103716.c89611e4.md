---
timestamp: 'Sun Oct 19 2025 10:37:16 GMT-0400 (Eastern Daylight Time)'
content_id: c89611e4ffa0d547f92b00ade5efaa88f1d4b22a952d585eadf08a1d0db0ef87
---

# file: src/concepts/PlaceDirectory/PlaceDirectoryConcept.test.ts
...
Deno.test("PlaceDirectoryConcept", async (t) => {
  ...
  await t.step(
    ...
    async () => {
      ...
      const zenTeaId = await placeDirectory.create_place({
        name: "Zen Tea House",
      ...
      const modernMatchaId = await placeDirectory.create_place({
      ...
      const distantCafeId = await placeDirectory.create_place({
      ...
      if (typeof zenTeaId === 'object' && 'error' in zenTeaId) {
        throw new Error("Failed to create Zen Tea House");
      }
      if (typeof modernMatchaId === 'object' && 'error' in modernMatchaId) {
        throw new Error("Failed to create Modern Matcha Bar");
      }
      if (typeof distantCafeId === 'object' && 'error' in distantCafeId) {
        throw new Error("Failed to create Distant Cafe");
      }

      // Step 2: Verify metadata is stored correctly
      const zenDetails = await placeDirectory.get_details({ placeId: zenTeaId });
      if (typeof zenDetails === 'object' && 'error' in zenDetails) {
        throw new Error("Failed to get Zen Tea details");
      }
      ...
      const matchaPlaces = await placeDirectory.search_by_name({ query: "Matcha" });
      ...
      const teaPlaces = await placeDirectory.search_by_name({ query: "Tea" });
      ...
      const nearbyPlaces = await placeDirectory.find_nearby({
        coords: [-74.006, 40.7128],
      ...
      const traditionalPlaces = await placeDirectory.filter_places({ style: "traditional" });
      ...
      const budgetPlaces = await placeDirectory.filter_places({ priceRange: "$" });
      ...
      const traditionalAndAffordable = await placeDirectory.filter_places({
        style: "traditional",
        priceRange: "$",
      });
